                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data.results.length > 0) {
                            var resultsHtml =
                                '<button id="downloadAllBtn" class="btn btn-success download-all-btn">Download All</button>';
                            resultsHtml +=
                                '<button id="ExportByExcel" class="btn btn-success download-all-btn">Export By Excel</button>';
                            resultsHtml += '<div class="total">Total Photes:' + data.results
                                .length + '</div>';
                            ...
                            $.each(data.results, function(index, result) {
                                resultsHtml += '<div class="col-md-6 mb-4">' +
                                    ...
                                    '<button class="download-btn" data-path="' + result
                                    .path + '">Download</button>' +
                                    '<div class="card-body">' +
                                    '<h5 class="card-text">Client: ' + result.client +
                                    '</h5>' +
                                    '<p class="card-text">State: ' + result.state +
                                    '</p>' +
                                    '<p class="card-text">Branch: ' + result.branch +
                                    '</p>' +
                                    '<p class="card-text">Address: ' + result.address +
                                    '</p>' +
                                    '<p class="card-text">Uploaded by: ' + result
                                    .employee_name + '</p>' +
                                    '</div>' +
                                    '</div>' +
                                    '</div>';
                            ...
                            $('#downloadAllBtn').click(function() {
                                ...
                                window.location.href =
                                    '{{ route('download.allImages') }}?paths=' +
                    ...
                    error: function() {
                        $('#adminResult').html(
                            '<div class="col-12"><p class="text-center">An error occurred while fetching the results.</p></div>'
                        );
                    }
                });
            });
